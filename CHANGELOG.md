...
- Reworked the application shell and added persistent Chinese/English switching.
- Unified public author and package metadata for HaoXiang Huang.
- Removed bundled third-party font binaries, font download APIs, and font packaging paths; document styles retain font family names only.
- Removed the legacy upstream AI endpoint fallback; default cloud configuration now requires an explicit Base URL.
- Added release checks for sensitive files, package metadata, and bundled font binaries.
...
- ✨ **默认内置配置**：历史远程默认地址（已在 1.4.7 clean public release 中移除）

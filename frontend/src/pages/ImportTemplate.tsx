...
interface ExtractResult {
  ...
  sections: Record<string, ExtractedSection | null>;
  page_setup: Record<string, JsonValue>;
  check_rules_count: number;
  fix_rules_count: number;
  yaml_preview: string;
  yaml_content: Record<string, JsonValue>;
}
...
function getErrorDetail(error: unknown, fallback: string): string {
  ...
  return String(apiError.response?.data?.detail || apiError.message || fallback);
}
...
export default function ImportTemplate() {
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      toastError('提取失败', getErrorDetail(err, '格式提取失败'));
    } finally {
      setExtracting(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toastError('保存失败', getErrorDetail(err, '保存失败'));
    } finally {
      setSaving(false);
    }
  };

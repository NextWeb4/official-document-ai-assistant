...
import {
  ...
  Layers, BookOpen, Building2, User, Plus, Search
} from 'lucide-react';
...
interface RuleDetail {
  key?: string;
  content?: RuleContent;
  source_type?: RuleItem['source_type'];
}
...
export default function Rules() {
  ...
  const handleDelete = async (rule: RuleItem) => {
    if (rule.source_type === 'official') {
      warning('提示', '官方规则为内置资源，不能删除');
      return;
    }
    if (!await confirm('确认删除', `确认删除规则“${rule.name}”？`)) return;
    try {
      await apiClient.delete(`/api/rules/${rule.key}?source_type=${rule.source_type}`);
      success('已删除', `规则“${rule.name}”已删除`);
      await loadRules();
    } catch (error: unknown) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      showError('错误', '导入失败: ' + getApiErrorDetail(error, '请重试'));
    }
  };
  ...
  const handleEnableToggle = async (rule: RuleItem) => {
    try {
      await apiClient.patch(`/api/rules/${rule.key}/enabled`, {
        source_type: rule.source_type,
        enabled: !rule.enabled,
      });
      await loadRules();
    } catch (error: unknown) {
      showError('更新失败', getApiErrorDetail(error, '请重试'));
      console.error('Toggle error:', error);
    }
  };

  const searchQuery = ruleSearch.trim().toLowerCase();
  const filteredRules = searchQuery
    ? rules.filter(rule =>
        rule.name.toLowerCase().includes(searchQuery) ||
        rule.key.toLowerCase().includes(searchQuery) ||
        rule.source_type.toLowerCase().includes(searchQuery)
      )
    : rules;
                ...
                <CardContent>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    <Button
                      ...
                      disabled={rule.source_type === 'official'}
                      title={rule.source_type === 'official' ? '官方规则为内置资源，不能删除' : '删除规则'}
                    >
                      <Trash2 className="h-4 w-4 mr-1" />
                      {rule.source_type === 'official' ? '内置不可删' : '删除'}
                    </Button>
                  </div>
                </CardContent>
              </Card>
                ...
                {selectedRule.content.check_rules && (
                  <div>
                    ...
                    <div className="space-y-2">
                      {selectedRule.content.check_rules.slice(0, 10).map((rule, i) => (

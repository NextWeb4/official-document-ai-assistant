...
import { parseDocumentId } from '@/lib/document-id.mjs';
...
export default function A4Preview() {
  ...
  const [error, setError] = useState(docId ? '' : '缺少文档 ID，无法加载预览');
  ...
  useEffect(() => {
    if (!docId) {
      void Promise.resolve().then(() => {
        setDocData(null);
        setError('缺少文档 ID，无法加载预览');
        setLoading(false);
      });
      return;
    }

    const parsedDocId = parseDocumentId(docId);
    if (parsedDocId === null) {
      void Promise.resolve().then(() => {
        setDocData(null);
        setError('文档 ID 无效，无法加载预览');
        setLoading(false);
      });
      return;
    }

    void Promise.resolve().then(() => loadDocument(parsedDocId));
  }, [docId, loadDocument]);
  ...
  const renderParagraph = (p: DocParagraph, idx: number) => {
    ...
    const align: React.CSSProperties['textAlign'] = p.format.alignment === 'justify'
      ? 'justify'

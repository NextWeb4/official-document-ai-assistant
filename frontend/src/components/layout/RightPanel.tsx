...
import {
  Activity, Cpu, Shield, Info, BookOpen, Sparkles,
...
function getCount(value: unknown, key?: 'rules') {
  if (Array.isArray(value)) return value.length;
  if (!value || typeof value !== 'object') return 0;
  const data = value as CountListResponse;
  if (key && Array.isArray(data[key])) return data[key].length;
  return typeof data.total === 'number' ? data.total : 0;
}

function getTemplates(value: unknown): TemplateSummary[] {
  if (Array.isArray(value)) return value as TemplateSummary[];
  if (!value || typeof value !== 'object') return [];
  const data = value as TemplateListResponse;
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (ruleR.status === 'fulfilled') {
        setRules(getCount(ruleR.value, 'rules'));
      }
    });
    return () => { ok = false; };
  }, []);
...
function DocAssistant() {
  ...
  useEffect(() => {
    apiClient.get('/api/templates/list')
      .then(r => setTemplates(getTemplates(r).slice(0, 3)))
      .catch(() => {});
  }, []);
...
function CheckStats() {
  ...
  useEffect(() => {
    ...
    apiClient.get(`/api/check/${docId}/results`).then(r => {
      const items: CheckIssueSummary[] = Array.isArray(r) ? r : [];

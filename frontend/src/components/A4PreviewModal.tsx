...
interface PreviewResponse {
  ...
  tables?: DocTable[];
  page_setup?: typeof DEFAULT_PAGE_SETUP;
}
...
export default function A4PreviewModal({ docId, templateId, templateName, refreshKey, onClose }: A4PreviewModalProps) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setPageSetup(resp.page_setup || DEFAULT_PAGE_SETUP);
    } catch (err: unknown) {
      const apiError = err as ApiError;
      const msg = apiError.response?.data?.detail || apiError.message || '预览加载失败';
      setError(typeof msg === 'string' ? msg : JSON.stringify(msg));
    } finally {
      setLoading(false);
    }
  }, [docId, isTemplateMode, templateId]);
  ...
  const renderP = (p: DocParagraph, key: number) => {
    ...
    const align = (p.format.alignment || 'left') as React.CSSProperties['textAlign'];

...
from core.rules.fixer import apply_fixes, apply_fixes_with_count
...
class RuleEngine:
    ...
    def check_and_fix_with_count(
        self,
        model: DocumentModel,
        doc_type: str,
        selected_rule_ids: list[str] | None = None,
    ) -> tuple[list[CheckIssue], DocumentModel, int]:
        """Check and fix while reporting successful fix handlers."""
        rules = self.load_rules(doc_type)
        issues = check_document(model, rules)
        logger.info(f"Found {len(issues)} issues before fixing")
        fixed_model, applied = apply_fixes_with_count(model, rules, selected_rule_ids)
        logger.info(f"Applied {applied} fixes for type: {doc_type}")
        return issues, fixed_model, applied

...
SUPPORTED_ACTIONS = frozenset(_ACTION_MAP)
...
def _apply_fixes(
    ...
    Args:
        ...
        selected_rule_ids: Check-rule IDs (`ref_check`) or fix-rule IDs to apply.
                          If None, apply all rules.

    Returns:
        A new DocumentModel and the number of handlers applied successfully.
    """
    import copy
    ...
    if selected_rule_ids is not None:
        ...
        fix_rules = [
            rule for rule in fix_rules
            if rule.get("id") in selected_set or rule.get("ref_check") in selected_set
        ]
...
def apply_fixes(
    ...
    """Apply fixes while preserving the original model-only public contract."""
    fixed, _ = _apply_fixes(model, rules, selected_rule_ids)
    return fixed


def apply_fixes_with_count(
    model: DocumentModel,
    rules: dict[str, Any],
    selected_rule_ids: list[str] | None = None,
) -> tuple[DocumentModel, int]:
    """Apply fixes and return the number of successful fix handlers."""
    return _apply_fixes(model, rules, selected_rule_ids)

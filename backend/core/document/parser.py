...
def parse_docx(file_path: Path | str) -> DocumentModel:
    ...
    for child in doc.element.body:
        ...
        if tag == 'p':
            ...
            table_positions.append(para_count - 1)

    tables = []
    for idx, table in enumerate(doc.tables):
        parsed_table = _parse_table(table, idx)
        # python-docx/lxml may return a new proxy for the same XML node, so
        # object identity cannot be used as a body-order key.
        parsed_table.insert_after_index = (
            table_positions[idx] if idx < len(table_positions) else -1
        )
...
def _parse_paragraph(para, index: int) -> Paragraph:
    ...
    if (
        not runs
        and para.text
        and para._element.find(qn("w:hyperlink")) is None
    ):
        runs.append(Run(
            text=para.text,
            index=0,
    ...
    # ---- 字体显示 fallback：保留段落/字符格式的直接值与继承值边界 ----
    _apply_style_font_fallback(para, runs)
...
def _apply_style_font_fallback(para, runs: list[Run]) -> None:
    """
    Fill preview font metadata from the style without materializing inherited
    paragraph or character toggles as direct formatting.
    """
    try:
        style = para.style
        if style is None:
...
def _parse_paragraph_format(para) -> ParagraphFormat:
            ...
            if pPr is not None:
                ...
                if spacing is not None:
                    ...
                    if line_val:
                        ...
                        xml_rule = spacing.get(qn('w:lineRule'))
                        if xml_rule == "auto":
                            line_spacing_pt = round(line_num / 240 * 16, 2)
                            line_spacing_rule = "multiple"
                        elif xml_rule == "atLeast":
                            line_spacing_pt = round(line_num / 20, 2)
                            line_spacing_rule = "atLeast"
                        else:
                            line_spacing_pt = round(line_num / 20, 2)
                            line_spacing_rule = "exact"
    ...
    return ParagraphFormat(
        ...
        space_before_pt=_safe_pt2(pf.space_before),
        space_after_pt=_safe_pt2(pf.space_after),
    )
...
def _parse_run(run, index: int) -> Run:
    ...
    latin_font = font_info.get("ascii") or font_info.get("hAnsi")
    ...
    return Run(
        text=run.text,
        index=index,
        preserved_xml=_preserved_run_xml(run),
        format=RunFormat(
...
def _preserved_run_xml(run) -> str | None:
    """Serialize a run when it contains OOXML not represented by Run.text."""
    modeled_children = {
        qn("w:rPr"),
        qn("w:t"),
        qn("w:tab"),
    ...
    if all(child.tag in modeled_children for child in run._element):
        return None

    from lxml import etree

    return etree.tostring(run._element, encoding="unicode")
...
def _parse_table(table, index: int) -> Table:
    """Parse one canonical anchor per logical cell, including merge spans."""
    cells: list[TableCell] = []
    active_vertical: dict[int, TableCell] = {}

    for row_idx, tr in enumerate(table._tbl.tr_lst):
        col_idx = _row_grid_before(tr)
        next_vertical: dict[int, TableCell] = {}

        for tc in tr.tc_lst:
            col_span = max(1, int(getattr(tc, "grid_span", 1) or 1))
            merge_state = getattr(tc, "vMerge", None)

            if merge_state == "continue":
                anchor = active_vertical.get(col_idx)
                same_anchor = anchor is not None and all(
                    active_vertical.get(col) is anchor
                    for col in range(col_idx, col_idx + col_span)
                )
                if (
                    same_anchor
                    and anchor.col == col_idx
                    and anchor.col_span == col_span
                ):
                    anchor.row_span += 1
                    for col in range(col_idx, col_idx + col_span):
                        next_vertical[col] = anchor
                    col_idx += col_span
                    continue

                logger.warning(
                    "Table %s has an orphan vertical-merge continuation at (%s,%s)",
                    index,
                    row_idx,
                    col_idx,
                )
            ...
            if merge_state in {"restart", "continue"}:
                for col in range(col_idx, col_idx + col_span):
                    next_vertical[col] = anchor
...
def _row_grid_before(tr) -> int:
    """Return the number of omitted grid columns before a table row."""
    tr_pr = tr.trPr
    if tr_pr is None:
        return 0

...
class RunFormat(BaseModel):
    ...
    latin_font_name: Optional[str] = None      # 西文/数字字体，写入 w:ascii/w:hAnsi
...
class Run(BaseModel):
    ...
    preserved_xml: Optional[str] = Field(default=None, exclude=True, repr=False)
...
class TableCell(BaseModel):
    """A logical table cell anchored at its top-left grid coordinate.

    Merged aliases are never represented as additional cells. ``row_span`` and
    ``col_span`` describe the rectangular grid region owned by this anchor.
    """
    row: int
    col: int
    text: str
...
class Table(BaseModel):
    """A table whose body order is anchored to a paragraph gap."""
    index: int
    rows: int
    cols: int
    cells: list[TableCell] = Field(default_factory=list)
    # Canonical body-order anchor; structural paragraph edits must remap it.
    insert_after_index: int = -1  # -1 means before the first body paragraph

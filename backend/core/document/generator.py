...
def generate_docx(model: DocumentModel, output_path: Path | str) -> Path:
    ...
    modeled_table_elements = _update_tables(doc, model)
    _restore_table_positions(doc, model, modeled_table_elements)
...
def _replace_paragraph_content(doc: Document, p_element, para_model: Paragraph):
    """
    替换一个 <w:p> 元素的内容，按模型顺序重放文本和保留的原子 runs。
    """
    para_obj = _find_paragraph_object(doc, p_element)
    original_text = para_obj.text if para_obj is not None else ""
    content_changed = original_text != para_model.text
    removed_relationship_ids: set[str] = set()
    retained_hyperlink = False

    # 原子 runs 已由 parser 序列化到模型中；旧 runs 必须全部清除，避免图片重复。
    for child in list(p_element):
        ...
        if tag == 'r':
            ...
            if content_changed:
                ...
                retained_hyperlink = True

    if removed_relationship_ids:
        _drop_unreferenced_relationships(doc.part, removed_relationship_ids)

    # 更新段落属性 (w:pPr)
    _update_pPr(p_element, para_model)

    # 获取对应的 python-docx Paragraph 对象以使用 API 添加 run
    if retained_hyperlink and not para_model.runs:
        return
...
def _update_pPr(p_element, para_model: Paragraph):
    ...
    if has_indent:
        ...
        if fmt.first_line_indent_pt is not None:
            ...
            base_font_pt = _paragraph_base_font_size(para_model)
            chars = int(round(fmt.first_line_indent_pt / base_font_pt * 100))
            if chars > 0:
...
def _drop_unreferenced_relationships(part, relationship_ids: set[str]) -> None:
    """Drop removed hyperlink relationships only when no body XML still references them."""
    referenced_values = {
        value
    ...
    for relationship_id in relationship_ids:
        if relationship_id in referenced_values:
            continue
...
def _add_runs_to_paragraph(para, para_model: Paragraph):
    """Append modeled runs, restoring opaque OOXML runs in their original order."""
    if para_model.runs:
        for run_model in para_model.runs:
            if run_model.preserved_xml:
                run_element = _build_preserved_run_element(run_model)
                para._element.append(run_element)
                _apply_run_format(_python_docx_run(run_element, para), run_model)
            else:
...
def _python_docx_run(run_element, parent):
    """Wrap a restored CT_R so existing format helpers can update it safely."""
    from docx.text.run import Run as DocxRun

    return DocxRun(run_element, parent)


def _build_preserved_run_element(run_model: Run):
    """Restore opaque run XML and apply any modeled text change without dropping atoms."""
    try:
        run_element = parse_xml(run_model.preserved_xml)
    except Exception as exc:
        raise ValueError("Invalid preserved inline OOXML") from exc
    if run_element.tag != qn("w:r"):
        raise ValueError("Preserved inline OOXML must contain a w:r root")

    _replace_modeled_text_in_preserved_run(run_element, run_model.text)
    return run_element


def _replace_modeled_text_in_preserved_run(run_element, text: str) -> None:
    """Replace w:t/tab/break payload while retaining drawing and field children."""
    ...
    if "".join(current_parts) == text:
        return

    if text_children:
        insertion_index = run_element.index(text_children[0])
    else:
        first_child = run_element[0] if len(run_element) else None
        insertion_index = 1 if first_child is not None and first_child.tag == qn("w:rPr") else 0
    for child in text_children:
        run_element.remove(child)
    for offset, child in enumerate(_text_to_run_children(text)):
        run_element.insert(insertion_index + offset, child)


def _text_to_run_children(text: str) -> list:
    """Convert modeled run text to w:t/w:tab/w:br children."""
    children = []
    buffer: list[str] = []
    ...
    for char in text:
        if char == "\t":
            flush_buffer()
            children.append(OxmlElement("w:tab"))
        elif char in ("\r", "\n"):
            flush_buffer()
            children.append(OxmlElement("w:br"))
        else:
            buffer.append(char)
    flush_buffer()
    return children
...
def _add_runs_via_xml(p_element, para_model: Paragraph):
    ...
    if para_model.runs:
        for run_model in para_model.runs:
            if run_model.preserved_xml:
                r = _build_preserved_run_element(run_model)
                _apply_run_format(_python_docx_run(r, None), run_model)
                p_element.append(r)
                continue
            r = OxmlElement('w:r')
            # 添加 run 格式属性
            ...
            if fmt.font_name:
                # 使用统一的字体设置，区分拉丁/中文字体
                rFonts = OxmlElement('w:rFonts')
                latin_font = fmt.latin_font_name or LATIN_FONT
                rFonts.set(qn('w:ascii'), latin_font)
                rFonts.set(qn('w:hAnsi'), latin_font)
...
def _update_tables(doc: Document, model: DocumentModel):
    ...
    for idx, table_model in enumerate(model_tables):
        if idx < len(existing_tables):
            ...
            modeled_table_elements.append(existing_tables[idx]._tbl)
        else:
            # 添加新表格
            table = _add_table(doc, table_model)
            if table is not None:
                modeled_table_elements.append(table._tbl)

    logger.debug(f"Updated {min(len(model_tables), len(existing_tables))} tables, "
                 f"added {max(0, len(model_tables) - len(existing_tables))}")
    return modeled_table_elements


def _restore_table_positions(doc: Document, model: DocumentModel, table_elements) -> None:
    """Move only tables whose paragraph-gap anchor changed."""
    body = doc.element.body
    paragraph_elements = [child for child in body if child.tag == qn('w:p')]
    modeled_count = min(len(model.tables), len(table_elements))

    if modeled_count != len(model.tables):
        logger.warning(
            "Cannot restore all table positions: model=%s document=%s",
            len(model.tables),
            len(table_elements),
        )
    ...
    for table_model, table_element in zip(
        model.tables[:modeled_count],
        table_elements[:modeled_count],
    ):
        if paragraph_count == 0:
            ...
            position = max(
                -1,
                min(table_model.insert_after_index, paragraph_count - 1),
            )
        tables_after.setdefault(position, []).append(table_element)

    for position in range(-1, paragraph_count):
        previous_table = None
        for table_element in tables_after.get(position, []):
            current_position = _table_paragraph_anchor(body, table_element)
            out_of_order = (
                previous_table is not None
                and body.index(table_element) < body.index(previous_table)
            )
            if current_position != position or out_of_order:
                if previous_table is not None:
                    previous_table.addnext(table_element)
                elif paragraph_elements:
                    if position == -1:
                    ...
                    body.insert(insert_at, table_element)
            previous_table = table_element


def _table_paragraph_anchor(body, table_element) -> int:
    """Return the index of the last direct body paragraph before a table."""
    paragraph_index = -1
    for child in body:
...
def _add_table(doc: Document, table_model: TableModel):
    ...
    try:
        ...
        _apply_table_merges(table, table_model)
...
def _apply_table_merges(table, table_model: TableModel) -> None:
    """Rebuild rectangular merges from canonical top-left cell anchors."""
    occupied: dict[tuple[int, int], tuple[int, int]] = {}

    for cell_model in sorted(table_model.cells, key=lambda cell: (cell.row, cell.col)):
        row_end = cell_model.row + cell_model.row_span - 1
        col_end = cell_model.col + cell_model.col_span - 1
        if (
            cell_model.row < 0
            or cell_model.col < 0
            or row_end >= len(table.rows)
            or col_end >= len(table.columns)
        ):
            logger.warning(
                "Skipping out-of-bounds table cell anchor (%s,%s) span %sx%s",
                cell_model.row,
                cell_model.col,
                cell_model.row_span,
                cell_model.col_span,
        ...
        region = [
            (row, col)
            for row in range(cell_model.row, row_end + 1)
            for col in range(cell_model.col, col_end + 1)
        ]
        conflicts = [coordinate for coordinate in region if coordinate in occupied]
        if conflicts:
            logger.warning(
                "Skipping overlapping table cell anchor (%s,%s); region already owned by %s",
                cell_model.row,
                cell_model.col,
                occupied[conflicts[0]],
            )
            continue

        anchor_coordinate = (cell_model.row, cell_model.col)
        for coordinate in region:
            occupied[coordinate] = anchor_coordinate

        if cell_model.row_span == 1 and cell_model.col_span == 1:
            continue

        table.cell(cell_model.row, cell_model.col).merge(table.cell(row_end, col_end))
...
def _update_hf_list(doc: Document, hf_models: list[HeaderFooter], hf_type: str):
    ...
    for hf_model in hf_models:
        try:
            ...
            if hf_type == "footer" and hf_model.has_page_number and hf_model.paragraphs:
                page_number_idx = _find_page_number_paragraph(hf_model.paragraphs)
            ...
            if hf_model.paragraphs:
                for p_idx, para_model in enumerate(hf_model.paragraphs):
                    if p_idx < len(target.paragraphs):
                        ...
                        for run in list(para.runs):
                            run._element.getparent().remove(run._element)
                        # 添加新 runs
                        if p_idx == page_number_idx and not _has_preserved_complex_field(para_model):
                            _add_page_number_field(para, para_model.text, para_model.runs)
                        else:
                            _add_runs_to_paragraph(para, para_model)
                        # 更新段落格式（缩进、行距、对齐等）
                        _update_pPr(para._element, para_model)
                ...
                if target.paragraphs:
                    ...
                    for run in list(para.runs):
                        run._element.getparent().remove(run._element)
                    if hf_type == "footer" and hf_model.has_page_number:
                        _add_page_number_field(para, hf_model.text, [])
                    else:
...
def _has_preserved_complex_field(para_model: Paragraph) -> bool:
    """Return whether parser-captured fldChar/instrText runs can be replayed directly."""
    for run_model in para_model.runs:
        if not run_model.preserved_xml:
            continue
        try:
            run_element = parse_xml(run_model.preserved_xml)
        except Exception:
            continue
        if (
            run_element.find(f".//{qn('w:fldChar')}") is not None
            or run_element.find(f".//{qn('w:instrText')}") is not None
        ):
            return True
    return False


def _find_page_number_paragraph(paragraphs: list[Paragraph]) -> int:
    """Locate the footer paragraph that carries the page-number placeholder."""
    for idx, para in enumerate(paragraphs):
        if para.role == "page_number":
            return idx

    for idx, para in enumerate(paragraphs):
        text = para.text or "".join(run.text for run in para.runs)
...
def _add_page_number_field(para, text: str, run_models: list[Run]) -> None:
    """Write a PAGE field with a cached result while retaining surrounding text."""
    text = text or "".join(run.text for run in run_models)
    matches = list(re.finditer(r"\d+", text))
    if matches:
        placeholder = matches[-1]
        prefix = text[:placeholder.start()]
        suffix = text[placeholder.end():]
        display_result = placeholder.group()
    elif text:
        prefix = text
        suffix = ""
        display_result = "1"
    ...
    for field in list(para._element.findall(f".//{qn('w:fldSimple')}")):
        if "PAGE" in (field.get(qn("w:instr"), "")).upper():
            parent = field.getparent()
            if parent is not None:
                parent.remove(field)

    format_model = next((run for run in run_models if run.text.strip()), None)
    if format_model is None and run_models:
        format_model = run_models[0]

    def add_run(run_text: str = ""):
        run = para.add_run(run_text)
        if format_model is not None:
            _apply_run_format(run, format_model)
        else:
            set_run_font(run, BODY_FONT)
        return run
    ...
    add_run()._element.append(begin)

    instruction = OxmlElement("w:instrText")
    instruction.set(qn("xml:space"), "preserve")
    instruction.text = " PAGE "
    add_run()._element.append(instruction)

    separate = OxmlElement("w:fldChar")
    separate.set(qn("w:fldCharType"), "separate")
    add_run()._element.append(separate)

    add_run(display_result)

    end = OxmlElement("w:fldChar")
    end.set(qn("w:fldCharType"), "end")
    add_run()._element.append(end)
...
def _apply_paragraph_format(para, para_model: Paragraph):
    ...
    if fmt.first_line_indent_pt is not None:
        ...
        _set_first_line_chars(para._element, fmt.first_line_indent_pt, _paragraph_base_font_size(para_model))
    if fmt.left_indent_pt is not None:
...
def _paragraph_base_font_size(para_model: Paragraph) -> float:
    """Return the first explicit run font size, used to convert pt indentation to character units."""
    for run in para_model.runs:
        if run.format.font_size_pt:
            return max(1.0, float(run.format.font_size_pt))
    return 16.0


def _set_first_line_chars(p_element, indent_pt: float, base_font_pt: float) -> None:
    """Mirror first-line indent as w:firstLineChars for Chinese document layout stability."""
    pPr = p_element.find(qn('w:pPr'))
    if pPr is None:
        pPr = OxmlElement('w:pPr')
        p_element.insert(0, pPr)
    ...
    if ind is None:
        ind = OxmlElement('w:ind')
        pPr.append(ind)
    chars = int(round(indent_pt / max(base_font_pt, 1.0) * 100))
    if chars > 0:
        ind.set(qn('w:firstLineChars'), str(chars))
...
def _apply_run_format(run, run_model: Run):
    ...
    # === 字体设置（统一入口） ===
    if fmt.font_name:
        set_run_font(run, fmt.font_name, fmt.latin_font_name)
    else:
        set_run_font(run, BODY_FONT, fmt.latin_font_name)

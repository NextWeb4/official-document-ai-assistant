...
def _select_paragraphs(model: DocumentModel, target: str) -> list[Paragraph]:
    ...
    if target == "title":
        ...
        doc_titles = [p for p in model.paragraphs if p.is_heading and p.heading_level == 0]
        if doc_titles:
            return doc_titles
        first_level_heading = next(
            (p for p in model.paragraphs if p.is_heading and p.heading_level == 1),
            None,
        )
        if first_level_heading:
            return [first_level_heading]
        first_heading = next((p for p in model.paragraphs if p.is_heading), None)
        return [first_heading] if first_heading else []
    elif target == "heading_1":
        headings = [p for p in model.paragraphs if p.is_heading and p.heading_level == 1]
        if any(p.is_heading and p.heading_level == 0 for p in model.paragraphs):
            return headings
        first_non_empty = next((p for p in model.paragraphs if p.text.strip()), None)
        return [p for p in headings if p is not first_non_empty]
        ...
        if len(non_empty) >= 4:
            sig_set = set(id(p) for p in non_empty[-2:])
        elif len(non_empty) >= 3:
            sig_set = {id(non_empty[-1])}
        else:
            sig_set = set()
...
def remove_extra_blank_lines(model: DocumentModel) -> None:
    """Remove consecutive blank paragraph blocks without crossing tables."""
    to_remove: set[int] = set()
    previous_block_was_blank = False
    for block_type, block_index in _iter_body_blocks(model):
        if block_type == "table":
            previous_block_was_blank = False
            continue

        is_blank = not model.paragraphs[block_index].text.strip()
        if is_blank and previous_block_was_blank:
            to_remove.add(block_index)
        previous_block_was_blank = is_blank

    _remove_paragraph_indices(model, to_remove)


def _iter_body_blocks(model: DocumentModel):
    """Yield paragraph and table references in their canonical body order."""
    paragraph_count = len(model.paragraphs)
    tables_after: dict[int, list[int]] = {}
    for table_index, table in enumerate(model.tables):
        position = table.insert_after_index
        if paragraph_count == 0:
            position = -1
        else:
            position = max(-1, min(position, paragraph_count - 1))
        tables_after.setdefault(position, []).append(table_index)

    for table_index in tables_after.get(-1, []):
        yield "table", table_index

    for paragraph_index in range(paragraph_count):
        yield "paragraph", paragraph_index
        for table_index in tables_after.get(paragraph_index, []):
            yield "table", table_index


def _remove_paragraph_indices(model: DocumentModel, indices) -> None:
    """Remove body paragraphs and keep every table anchored to the same block gap."""
    removed = sorted({index for index in indices if 0 <= index < len(model.paragraphs)})
    if not removed:
        return

    for table in model.tables:
        if table.insert_after_index >= 0:
            removed_before_table = sum(
                1 for index in removed if index <= table.insert_after_index
            )
            table.insert_after_index -= removed_before_table
    ...
    last_paragraph_index = len(model.paragraphs) - 1
    for table in model.tables:
        table.insert_after_index = max(
            -1,
            min(table.insert_after_index, last_paragraph_index),
        )
    ...
    for para in model.paragraphs:
        ...
        for run in para.runs:
            ...
            if run_end <= split_pos:
                ...
                run.text = first_part
                new_format = copy.deepcopy(run.format)
...
def _replace_paragraph_range(
    ...
    """Replace a text range without collapsing unaffected run formatting."""
    original_text = para.text
    if not (0 <= start <= end <= len(original_text)):
        raise ValueError("paragraph replacement range is out of bounds")

    if not para.runs:
        para.text = original_text[:start] + replacement + original_text[end:]
        para.runs = [Run(index=0, text=para.text, format=RunFormat())]
        return

    run_text = "".join(run.text for run in para.runs)
    if run_text != original_text:
        logger.warning(
            "Paragraph text/run mismatch during range replacement; "
            "preserving the first run format"
        )
        para.text = original_text[:start] + replacement + original_text[end:]
        first_format = copy.deepcopy(para.runs[0].format)
        para.runs = [Run(index=0, text=para.text, format=first_format)]
        return

    replacement_format = copy.deepcopy(para.runs[-1].format)
    offset = 0
    for run in para.runs:
        run_end = offset + len(run.text)
        if start < run_end or (start == len(original_text) and run_end == start):
            replacement_format = copy.deepcopy(run.format)
            break
        offset = run_end

    prefix_runs: list[Run] = []
    suffix_runs: list[Run] = []
    offset = 0
    for run in para.runs:
        run_end = offset + len(run.text)
        prefix_length = max(0, min(len(run.text), start - offset))
        if prefix_length:
            preserved = copy.deepcopy(run)
            preserved.text = run.text[:prefix_length]
            prefix_runs.append(preserved)

        suffix_start = max(0, min(len(run.text), end - offset))
        if suffix_start < len(run.text):
            preserved = copy.deepcopy(run)
            preserved.text = run.text[suffix_start:]
            suffix_runs.append(preserved)
        offset = run_end

    new_runs = prefix_runs
    if replacement:
        new_runs.append(Run(index=0, text=replacement, format=replacement_format))
    new_runs.extend(suffix_runs)
...
def normalize_heading_content(model: DocumentModel) -> int:
    ...
    for para in model.paragraphs:
        if not para.text.strip():
            continue
        leading_length = len(para.text) - len(para.text.lstrip())
        ...
        if m and para.is_heading and (para.heading_level == 1 or para.heading_level is None):
            ...
            if cn:
                new_prefix = f'{cn}、'
                if new_prefix != m.group(0):
                    _replace_paragraph_range(
                        para,
                        leading_length,
                        leading_length + m.end(),
                        new_prefix,
                    )
                    changes += 1
                continue
...
def replace_paragraph_text(model: DocumentModel, para_index: int, new_text: str) -> None:
    ...
    if 0 <= para_index < len(model.paragraphs):
        ...
        para.text = new_text
        replacement_format = (
            copy.deepcopy(para.runs[0].format) if para.runs else RunFormat()
        )
        para.runs = [Run(index=0, text=new_text, format=replacement_format)]

...
def classify_with_ai(model: DocumentModel, provider_name: str = "ollama") -> bool:
    """
    使用AI模型对文档段落进行语义分类。
    ...
    try:
        ...
        from config import is_local_base_url, is_offline_mode
    ...
    try:
        ...
        if not config:
            ...
            return False
        if is_offline_mode() and not is_local_base_url(config.base_url or ""):
            logger.info("Offline mode: skipping non-local AI structure analysis")
            return False

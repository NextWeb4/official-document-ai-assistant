...
路径分层：
  BASE_DIR     — 只读资源（rules/official, templates/official）
...
if getattr(sys, 'frozen', False):
    ...
    # BASE_DIR 指向上级 resources/ 目录（与 rules/、templates/ 同级）
...
def get_app_mode() -> str:
    """Return current app mode. Defaults to offline/local."""
    mode = os.getenv("APP_MODE", APP_MODE).strip().lower()
    return "online" if mode == "online" else "offline"


def is_offline_mode() -> bool:
    return get_app_mode() == "offline"


def is_local_base_url(base_url: str) -> bool:
    """True when a URL targets loopback only."""
    if not base_url:
        return False

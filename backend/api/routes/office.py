...
_TEMPLATE_READ_SOURCES = frozenset({"all", "official", "custom", "user"})
_TEMPLATE_FORMATS = frozenset({"docx", "dotx"})
TemplateReadSource = Literal["all", "official", "custom", "user"]
TemplateOutputFormat = Literal["docx", "dotx"]
...
async def office_fix(payload: DocumentPayload):
    ...
    try:
        ...
        # 生成修复后的文档
        out_path = _BRIDGE_TMP / f"fixed_{_safe_docx_filename(payload.filename)}"
...
async def office_list_templates(source: TemplateReadSource = "all"):
    """获取样式模板列表。"""
    if source not in _TEMPLATE_READ_SOURCES:
        raise HTTPException(
            status_code=400,
...
async def office_apply_template(payload: TemplateApplyRequest):
    ...
    将模板的样式定义应用到现有文档的格式上。
    """
    template_id = _safe_template_id(payload.template_id)
    try:
        doc_path = _decode_to_temp(payload.document_base64, payload.filename)
        model = parse_docx(doc_path)
        ...
        _, fixed_model = _rule_engine.check_and_fix(model, template_id)

        out_path = _BRIDGE_TMP / f"styled_{_safe_docx_filename(payload.filename)}"
        generate_docx(fixed_model, out_path)
        ...
        return {
            ...
            "message": f"已应用 {template.get('name', template_id)} 样式",
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def office_generate_template(
    template_id: str,
    format: TemplateOutputFormat = "docx",
):
    """
    生成模板文件下载。
    ...
    if format not in _TEMPLATE_FORMATS:
        raise HTTPException(status_code=400, detail="Template format must be docx or dotx")
...
def _safe_docx_filename(filename: str) -> str:
    ...
    return safe_name if safe_name not in ("", ".", "..") else "document.docx"


def _safe_template_id(template_id: str) -> str:
    cleaned = (template_id or "").strip()
    if not _TEMPLATE_ID_RE.fullmatch(cleaned):
        raise HTTPException(
            status_code=400,
            detail="Template id may only contain letters, numbers, underscores, and hyphens",
        )
    return cleaned
